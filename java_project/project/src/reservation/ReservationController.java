...
public class ReservationController {
	...
	public void add() {
		...
		String date = inputC.validInputEntranceTime("날짜와 시간을 입력하세요.");
		...
		while (avail(date, tid)) {
			...
			System.out.println();
			date = inputC.validInputEntranceTime("날짜와 시간을 입력하세요.");
	...
	void show() {
		list = rDao.show(conn);
		int rId = inputC.checkUserChoice("조회할 예약 번호를 입력하세요.", 1, list.size()+1);
		System.out.println();

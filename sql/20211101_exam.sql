...
-- 평균에 대해서는 정수로 반올림하시오.
select max(sal) as "최고액", min(sal) as "최저액", to_char(sum(sal), 'L999,999,999') as "총액", to_char(avg(sal), 'L999,999,999') as "평균 급여"
from emp;

-- 24. 각 담당 업무 유형별로 급여 최고액, 최저액, 총액 및 평균 액을 출력하시오.
-- 평균에 대해서는 정수로 반올림 하시오.
select job, 
to_char(max(sal), 'L999,999,999') as "최고액", 
to_char(min(sal), 'L999,999,999') as "최저액", 
to_char(sum(sal), 'L999,999,999') as "총액", 
to_char(avg(sal), 'L999,999,999') as "평균 급여"
from emp
group by job;
...
select deptno, count(*), to_char(avg(sal), 'L999,999,999') as "평균 급여"
from emp
group by deptno;
...
select job, deptno as dno, nvl(decode(deptno, 10, to_char(sum(sal), 'L999,999,999')) as "10번 부서",
                           nvl(decode(deptno, 20, to_char(sum(sal), 'L999,999,999')) as "20번 부서",
                           nvl(decode(deptno, 30, to_char(sum(sal), 'L999,999,999')) as "30번 부서",
                           sum(sal) as "총액"
from emp
group by job, deptno
order by deptno, job;